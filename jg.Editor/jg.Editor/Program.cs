...
namespace jg.Editor
{
    public class Program
    {
        ...
        static void Main(string[] args)
        {
          ...
          string AssembName=  Assembly.GetExecutingAssembly().GetName().Name.ToString();
     
            if (UpdateAppconfig(Globals.appStartupPath + "\\" + "jg.UpdateClient.exe.config", "IsUpdate", "").ToLower() == ("true"))
            {
                byte[] bs = global::jg.Editor.Properties.Resources.jg_UpdateClient;

                Assembly asm = Assembly.Load(bs);
                FileStream f = new FileStream(Globals.appStartupPath + "\\" + UpdatePath, FileMode.Create);
                f.Write(bs, 0, bs.Count());
                ...
                if (File.Exists(Globals.appStartupPath + "\\" + UpdatePath))
                {
                    ...
                    processUpdate = Process.Start(Globals.appStartupPath + "\\" + UpdatePath, key + " " + version + " " + id + " " +Globals.appStartupPath + "\\" + "AssembName.exe");
        ...
        static string UpdateAppconfig(string appconfigPath, string appKey, string appValue)
        {
            ...
            if (xElem1 != null)
            {
                content = xElem1.GetAttribute("value");
            }
            return content;
        }
        //设置jgx文件格式的注册表
