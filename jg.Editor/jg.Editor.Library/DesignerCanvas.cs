namespace jg.Editor.Library
{
    ...
    public class DesignerCanvas : Canvas
    {
        ...
        public delegate void addText(Point p);
        public static event addText addTextHander = null;
        public delegate void OnMenuClick(string Header);

        private bool IsText = false;

        public static event OnMenuClick OnMenuClickHander = null;
        ...
        protected override void OnInitialized(EventArgs e)
        {
            ...
            MenuItem m = new MenuItem() { Header = "新建" };
            m.Click += m_Click;
            context.Items.Add(m);
            MenuItem m1 = new MenuItem() { Header = "左对齐" };
            m1.Click += m_Click;
            context.Items.Add(m1);
            MenuItem m2 = new MenuItem() { Header = "右对齐" };
            m2.Click += m_Click;
            context.Items.Add(m2);
            MenuItem m3 = new MenuItem() { Header = "底部对齐" };
            m3.Click += m_Click;
            context.Items.Add(m3);
            MenuItem m4 = new MenuItem() { Header = "顶端对齐" };
            m4.Click += m_Click;
            context.Items.Add(m4);
            //MenuItem m5 = new MenuItem() { Header = "新建" };
            //context.Items.Add(m5);
        ...
        void m_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (OnMenuClickHander != null)
            {
                OnMenuClickHander(m.Header.ToString());
            }
        ...
        {
            base.OnMouseDown(e);
            if (addTextHander != null)
            {
             
                addTextHander(  Mouse.GetPosition(e.Source as FrameworkElement));
               return;
            }
        ...
        public void AddItem(string xamlString, string AssetPath, string Thumbnails, string itemsDis)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(xamlString))
                {
                    ...
                    item.WaterMakeThumbnails = Thumbnails;
                    item.WaterMakeAssetPath = AssetPath;
                    ...
                    switch (xamlString)
                    {
                        ...
                        case "TopicDrag":
                            ...
                            ControlTopicDrag controlTopicDarg = new ControlTopicDrag(topicDragItemAnswerList, topicDragItemList, true,false);
                        ...
                        case "Model3D":
                            ...
                            image = new Image() { Source = bipImg };
                            image.Stretch = Stretch.Fill;
                            item.AssetType = AssetType.Model3D;
                            item.Content = image;
                            break;
                        case "Animation":
                            image = new Image();
                        ...
                        case "Line":
                            ...
                            ControlLine controlLine = new ControlLine(ControlLineType.Line);
                        ...
                        default:
                            ...
                            {
                                UserControl usercontrol = item.Content as UserControl;


                                if (usercontrol is shapeRect)
                                {
                                    item.AssetType = AssetType.Shape;
                                ...
                                //bWidth.Source = item.Width;
                                //bWidth.UpdateSourceTrigger = UpdateSourceTrigger.PropertyChanged;
                                //usercontrol.SetBinding(UserControl.WidthProperty, bWidth);

                                //Binding bHeight = new Binding();
                                //bHeight.Source = item.Height;
                                //bHeight.UpdateSourceTrigger = UpdateSourceTrigger.PropertyChanged;
                                //usercontrol.SetBinding(UserControl.HeightProperty, bHeight);
        ...
        /// <param name="Thumbnails"></param>
        /// <param name="itemsDis"></param>
        public void AddItem(string xamlString, string AssetPath, string Thumbnails, string itemsDis,float type)
        {
            addTextHander = null;
            try
            {
                  ...
                  double pixelweight = 0;
                    switch (xamlString)
                    {
                        ...
                        case "Image":
                            ...
                            pixelheight = bipImg.PixelHeight;
                            pixelweight = bipImg.PixelWidth;
                            item.AssetType = AssetType.Image;
                            item.Content = image;
                ...
                if (item != null)
                {
                    ...
                    if (this.ActualWidth - newItem.Width <= 0)
                    {
                        left = 0;
                    }
                    else
                    {
                        left = this.ActualWidth - newItem.Width;
                    }

                    if (this.ActualHeight - newItem.Height <= 0)
                    {
                        top = 0;
                    }
                    else
                    {
                        top = this.ActualHeight - newItem.Height;
                    }
                    DesignerCanvas.SetLeft(newItem, left);
                    DesignerCanvas.SetTop(newItem, top);
                    newItem.ItemName = item.AssetType.ToString() + base.Children.Count.ToString();
                    base.Children.Add(newItem);
                    newItem.PropertyChanged += new PropertyChangedEventHandler(this.newItem_PropertyChanged);
                    Panel.SetZIndex(newItem, this.Children.Count);
        ...
        public void AddItem(string xamlString, string AssetPath, string Thumbnails, string itemsDis, ObservableCollection<TopicDragItemInfo> topicDragItemList, ObservableCollection<TopicDragItemAnswerInfo> topicDragItemAnswerList)
        {                                                                                          
            addTextHander = null;
            try
            {
        ...
        public void AddItem(string xamlString, string AssetPath, string Thumbnails, string itemsDis,TopicInfo ti)
        {
            addTextHander = null;
            try
            {
        ...
        public void addTextPoint(Point p)
        {
            try
            {
                string TempPath;
                ...
                if (!String.IsNullOrEmpty("Text"))
                {
                    ...
                    item.AssetPath = "";
                    item.Thumbnails = "";
                    item.WaterMakeThumbnails = "";
                    item.WaterMakeAssetPath = "";
                    item.ItemsDis = "";
                ...
                if (item != null)
                {
                    ...
                    DesignerCanvas.SetLeft(newItem, p.X);
                    DesignerCanvas.SetTop(newItem, p.Y);
                    newItem.ItemName = item.AssetType.ToString() + base.Children.Count.ToString();
                    base.Children.Add(newItem);
                    newItem.PropertyChanged += new PropertyChangedEventHandler(this.newItem_PropertyChanged);
                    Panel.SetZIndex(newItem, this.Children.Count);
        ...
        public void AddItem(string xamlString)
        {
            addTextHander = null;
            addTextHander += addTextPoint;
